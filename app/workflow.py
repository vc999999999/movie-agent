...
class WorkflowRegistry:
    ...
    def select_workflow(
        ...
        shot: ShotSpec,
        aspect_ratio: str,
        available_vram_gb: int = 24,
        comfyui_installed_nodes: Optional[set[str]] = None,
        comfyui_installed_models: Optional[set[str]] = None,
    ) -> WorkflowPlan:
        candidates: list[WorkflowProfile] = []

        # 1. Match generation_mode
        for prof in self.profiles.values():
            if prof.generation_mode == shot.generation_mode:
        ...
        # 3. Check aspect ratio & frame count
        ...
        # 4. Check VRAM
        vram_filtered = [p for p in aspect_filtered if p.min_vram_gb <= available_vram_gb]
        if not vram_filtered:
            vram_filtered = aspect_filtered  # fallback if vram constraint is soft
            ...
            if comfyui_installed_nodes is not None:
                ...
                if missing_nodes:
                    return WorkflowPlan(
                        ...
                        reason=f"ComfyUI 环境缺少必要节点: {missing_nodes}",
                        required_capabilities=missing_nodes,
                    )

            if comfyui_installed_models is not None:
                missing_models = [m for m in prof.required_models if m not in comfyui_installed_models]
                if missing_models:
                    return WorkflowPlan(
                        shot_id=shot.shot_id,
                        workflow_id=prof.workflow_id,
                        status="precheck_failed",
                        reason=f"ComfyUI 环境缺少模型文件: {missing_models}",
                        required_capabilities=missing_models,
                    )
    ...
    def patch_workflow(
        ...
        profile = self.get_profile(workflow_id)
        patch_map = self.get_patch_map(workflow_id)
        raw_wf = self.get_raw_workflow(workflow_id)

        if not profile or not patch_map or not raw_wf:
            return {}, ErrorDetail(
        ...
        # Deep copy to ensure raw template is never modified
        patched = copy.deepcopy(raw_wf)
