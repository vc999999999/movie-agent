      ...
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          {activeStatus === "success" ? (
            <video
              src={outputUrl(projectId, String((run ?? renderShot.data)?.output_json?.file_path ?? `${pkg.shot_id}.mp4`).split("/").pop()!)}
              controls
              loop
              muted
              preload="metadata"
...
export function RenderStage({ projectId }: { projectId: string }) {
  ...
  async function resume() {
    try {
      setStartError(null);
      await apiJson(`/api/projects/${encodeSeg(projectId)}/auto_pipeline?render=true`, { method: "POST" });
      await pipeline.refetch();
    } catch (error) { setStartError(error instanceof Error ? error : new Error(String(error))); }
  }
  const controls = <><PipelineSwarm projectId={projectId} onStart={() => void resume()} />{startError ? <ErrorNotice title="启动失败" error={startError} /> : null}<ProductionTools projectId={projectId} /></>;
  ...
  if (packagesQuery.isError || !data || pkgs.length === 0) {
    return (
      <div style={{ display: "grid", gap: 16 }}>{controls}
