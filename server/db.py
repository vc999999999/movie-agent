...
class Database:
    ...
    def get_latest_artifact(self, project_id: str, kind: str) -> Optional[dict[str, Any]]:
        with self._conn() as conn:
            row = conn.execute(
                "SELECT * FROM artifacts WHERE project_id = ? AND kind = ? ORDER BY version DESC LIMIT 1",
                (project_id, kind)
            ).fetchone()
            if not row:
                return None
    ...
    def get_all_artifacts(self, project_id: str) -> list[dict[str, Any]]:
        with self._conn() as conn:
            rows = conn.execute("SELECT * FROM artifacts WHERE project_id = ? ORDER BY created_at ASC", (project_id,)).fetchall()
            result = []
            for r in rows:
                item = dict(r)
        ...
        with self._conn() as conn:
            conn.execute(
                ...
                (
                    ...
                    run["created_at"], run.get("finished_at")
                )
            )
        return run["id"]
    ...
    def list_render_runs_for_project(self, project_id: str) -> list[dict[str, Any]]:
        with self._conn() as conn:
            rows = conn.execute("SELECT * FROM render_runs WHERE project_id = ? ORDER BY created_at ASC", (project_id,)).fetchall()
            runs = []
            for r in rows:
                item = dict(r)
