...
async def chosen_project():
    ...
    await service.confirm_brief(pid, {"user_must_keep": ["红色围巾"], "genre": ["悬疑"]}, generate=False)
    package = await service.generate_treatments(pid)
    await service.confirm_treatment(pid, package.options[-1].treatment_id, generate=False)
...
async def test_explicit_selection_before_production():
    ...
    await service.analyze_input(pid)
    await service.confirm_brief(pid, generate=False)
    ...
    with pytest.raises(ValueError, match="导演方案"):
        await service.run_auto_pipeline(pid, render=False)
    with pytest.raises(ValueError, match="导演方案"):
...
async def test_brief_patch_preserves_unedited_fields_and_invalidates_downstream():
    ...
    await service.run_auto_pipeline(pid, render=False)
    await service.confirm_brief(pid, {"title": "新的片名"}, generate=False)
...
async def test_running_pipeline_blocks_edits_and_can_retry(monkeypatch):
    ...
    async def paused(project_id, **kwargs):
        entered.set()
        await release.wait()
