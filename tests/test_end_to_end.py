...
async def choose_and_generate(pid, evaluation_mode="full"):
    ...
    await project_service.run_auto_pipeline(pid, render=False, evaluation_mode=evaluation_mode)
...
async def test_case_2_60s_two_characters_with_dialogue():
    ...
    from agent.execution import QualityFailed
    # The one-character mock must not pass a two-character hard constraint.
    with pytest.raises(QualityFailed):
        await choose_and_generate(pid, evaluation_mode="rules")
    shots = project_service.get_shots(pid)
    total_dur = sum(s["duration_seconds"] for s in shots)
